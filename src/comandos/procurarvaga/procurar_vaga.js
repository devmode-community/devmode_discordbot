...
class RolarDadosComando extends commando.Command {
    constructor(client) {
        super(client, {
            ...
            group: 'procurarvaga',
            memberName: 'work',
            description: 'Montar uma apresentação resumida informando a procura por emprego'
        });
    }
                                                                ...
                                                                message.channel.awaitMessages(m => m.author.id == message.author.id,
                                                                    {max: 1, time: 60000}).then(collected => {
                                                                        if (collected.first().content.toLowerCase() === 'cancelar' ||
                                                                            ...
                                                                            let procurador = { nome, area, skills, link, contato };
                                                                            ...
                                                                            this.client.channels.cache.get(id_procurando_vagas).send(`Solicitado por: <@${message.author.id}>`)
                                                                            this.client.channels.cache.get(id_procurando_vagas).send({ embed: embed })
                                                                        }
